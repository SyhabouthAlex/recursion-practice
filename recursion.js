...
function longest(words, i=0, highest=0) {
  if (i >= words.length) return highest;
  if (words[i].length > highest) {
    return longest(words, i+1, words[i].length);
  }
  else {
    return longest(words, i+1, highest);
  }
}
...
function isPalindrome(str, first=0, last=str.length-1) {
  if (first === last || (last - first === 1 && str[first] === str[last])) return true;
